...
public class RestClient {
	...
	public void get(String url, String serviceUrl) {
		...
		RequestConfig config = RequestConfig.custom().setProxy(proxy).setConnectTimeout(10000).setSocketTimeout(10000)
				.build();
		...
		// GET request
		Response response = RestAssured.given().when().get(serviceUrl).then().statusCode(200).extract().response();
	...
	public void post(String url, String serviceUrl) {
		...
		Response response = RestAssured.given().body(requestBody.toString()).post(serviceUrl).then().statusCode(201)
				.extract().response();
